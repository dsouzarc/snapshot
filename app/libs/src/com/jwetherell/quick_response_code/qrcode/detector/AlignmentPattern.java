...
public final class AlignmentPattern extends com.google.zxing.ResultPoint {
     ...
     */
    com.jwetherell.quick_response_code.qrcode.detector.AlignmentPattern combineEstimate(float i, float j, float newModuleSize) {
        float combinedX = (getX() + j) / 2.0f;
        float combinedY = (getY() + i) / 2.0f;
        float combinedModuleSize = (estimatedModuleSize + newModuleSize) / 2.0f;
        return new com.jwetherell.quick_response_code.qrcode.detector.AlignmentPattern(combinedX, combinedY, combinedModuleSize);
    }
