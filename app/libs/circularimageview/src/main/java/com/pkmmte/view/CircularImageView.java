...
public class CircularImageView extends android.widget.ImageView {
	...
	private android.graphics.BitmapShader shader;
	private android.graphics.Bitmap image;
	private android.graphics.Paint paint;
	private android.graphics.Paint paintBorder;
	private android.graphics.Paint paintSelectorBorder;
	private android.graphics.ColorFilter selectorFilter;
	...
	private void init(android.content.Context context, android.util.AttributeSet attrs, int defStyle) {
		...
		paintBorder = new android.graphics.Paint();
		paintBorder.setAntiAlias(true);
		paintSelectorBorder = new android.graphics.Paint();
		...
		android.content.res.TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CircularImageView, defStyle, 0);
		...
		if(hasBorder) {
			...
			setBorderColor(attributes.getColor(R.styleable.CircularImageView_border_color, android.graphics.Color.WHITE));
		}

		// Set selector properties if enabled
		if(hasSelector) {
			int defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
			setSelectorColor(attributes.getColor(R.styleable.CircularImageView_selector_color, android.graphics.Color.TRANSPARENT));
			...
			setSelectorStrokeColor(attributes.getColor(R.styleable.CircularImageView_selector_stroke_color, android.graphics.Color.BLUE));
		}
	...
	public void setSelectorColor(int selectorColor) {
		this.selectorFilter = new android.graphics.PorterDuffColorFilter(selectorColor, android.graphics.PorterDuff.Mode.SRC_ATOP);
		this.invalidate();
	}
	...
	public void setShadow(boolean shadowEnabled) {
		...
		if(shadowEnabled) {
			...
			paintBorder.setShadowLayer(4.0f, 0.0f, 2.0f, android.graphics.Color.BLACK);
			paintSelectorBorder.setShadowLayer(4.0f, 0.0f, 2.0f, android.graphics.Color.BLACK);
		}
		else {
			//paint.setShadowLayer(0, 0.0f, 2.0f, Color.BLACK);
			paintBorder.setShadowLayer(0, 0.0f, 2.0f, android.graphics.Color.BLACK);
			paintSelectorBorder.setShadowLayer(0, 0.0f, 2.0f, android.graphics.Color.BLACK);
		}
	}
	...
	private int measureWidth(int measureSpec) {
		...
		else if (specMode == android.view.View.MeasureSpec.AT_MOST) {
	...
	private int measureHeight(int measureSpecHeight) {
		int result;
		int specMode = android.view.View.MeasureSpec.getMode(measureSpecHeight);
		int specSize = android.view.View.MeasureSpec.getSize(measureSpecHeight);
	...
	public android.graphics.Bitmap drawableToBitmap(android.graphics.drawable.Drawable drawable) {
		if (drawable == null)   // Don't do anything without a proper drawable
			return null;
		else if (drawable instanceof android.graphics.drawable.BitmapDrawable)    // Use the getBitmap() method instead if BitmapDrawable
	...
	public void refreshBitmapShader() {
		shader = new android.graphics.BitmapShader(android.graphics.Bitmap.createScaledBitmap(image, canvasSize, canvasSize, false), android.graphics.Shader.TileMode.CLAMP, android.graphics.Shader.TileMode.CLAMP);
	}
